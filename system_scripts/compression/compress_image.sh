...
if [ ${IMAGE_FORMAT} = "evc" ]; then
	...
	QP=$( ${VVC_ENC} -i ${TEMPFILEYUV} --profile main_10_still_picture --qpa 1 -f 1 -c yuv420 --internal-bitdepth 8 -t 2 -r 1 -b ${TARGET_SIZE} -s ${resolution} --preset medium -o  ${TEMPFILE} | grep "frame I" | grep "AvgQP" |  cut -d ":" -f 6 | cut -d "." -f 1 |  tr -dc '0-9' )
	echo "QP = ${QP}"
	...
	if [ "${QP}" -gt "40" ]; then
		...
		QP=$( ${VVC_ENC} -i ${TEMPFILEYUV} --profile main_10_still_picture --qpa 1 -f 1 --pass 2 --rcstatsfile ${RCFILE} -c yuv420 --internal-bitdepth 8 -t 2 -r 1 -b ${new_size} -s ${resolution} --preset medium -o  ${TEMPFILE} | grep "frame I" | grep "AvgQP" |  cut -d ":" -f 6 | cut -d "." -f 1 |  tr -dc '0-9' )
		echo "QP 3 = ${QP}"
	    fi
	    rm -f ${RCFILE}
	fi
